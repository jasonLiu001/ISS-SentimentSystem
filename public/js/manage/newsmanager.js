...
var options = {
    ...
    callback: function (pageIndex, element) {
        ...
        //todo:获取查询参数
        // beforeLoad();
        window.baseTools.GetManagerNews(parameters, function (result) {
...
var viewModel = function (data) {
    ...
    self.DeleteBat = function (obj) {
        ...
        //调用API
        if (objs.length > 0) {
            window.baseTools.DeleteNewsBat({
                query: { ids: objs }
            }, function (result) {
                if (result.Result) {
    ...
    self.UpdateBat = function (obj) {
        ...
        if (obj.b_id) {
            ...
            $("input.edit:checked").each(function () {
                var bid = $(this).attr("bid");
                var ntitle = $("#newsList tr td textarea[bid='" + bid + "']").val();
                var nscore = $("#newsList tr td input.score[bid='" + bid + "']").val();
                objs.push({ id: bid, title: ntitle, score: nscore });
            });
        }
        //调用API
        if (objs.length > 0) {
            window.baseTools.EditNewsBat({
                query: objs
            }, function (result) {
                if (result.Result) {
                    alert("更新成功！");
...
function startSearch() {
    ...
    beforeLoad();
    //todo:初始化查询参数
    window.baseTools.GetManagerNews(parameters, function (result) {//base.js //获取新闻舆情 
...
function setParameters() {
    ...
    if (searchKeys != "") {
        parameters.query.news_title = searchKeys;
    }
}
//初始化分页
...
function initGridList(result) {
    ...
    ko.applyBindings(viewm, document.getElementById("newsList"));
    if (result.rows.length == 0) {
        $("#newsList tbody").empty().append("<tr><td colspan='7'>很抱歉，没有相关数据！</td></tr>");
    }

    $("#newsList tfoot td").removeClass("loadingdata");
    $("#newsList tbody").fadeIn(300);
}
//加载样式
function beforeLoad() {
    $("#newsList tfoot td").addClass("loadingdata");
    $("#newsList tbody").hide();
}
function endLoad() {
    $("#newsList tfoot td").removeClass("loadingdata");
