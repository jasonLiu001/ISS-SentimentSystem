...
var options = {
    ...
    callback: function (pageIndex, element) {
        ...
        //todo:获取查询参数
        // beforeLoad();
        window.baseTools.GetManagerWeibo(parameters, function (result) {
...
var viewModel = function (data) {
    ...
    self.DeleteBat = function (obj) {
        ...
        //调用API
        if (objs.length > 0) {
            window.baseTools.DeleteWeiboBat({
                query: { ids: objs }
            }, function (result) {
                if (result.Result) {
    ...
    self.UpdateBat = function (obj) {
        ...
        if (obj.b_id) {
            var nid = obj.b_id;
            var newtitle = $("#weiboList tr td textarea[bid='" + nid + "']").val();
            obj.title = newtitle;
            var ntitle = newtitle;
            ...
            $("input.edit:checked").each(function () {
                var bid = $(this).attr("bid");
                var ntitle = $("#weiboList tr td textarea[bid='" + bid + "']").val();
                var nscore = $("#weiboList tr td input.score[bid='" + bid + "']").val();
                objs.push({ id: bid, title: ntitle, score: nscore });
            });
        }
        //调用API
        if (objs.length > 0) {
            window.baseTools.EditWeiboBat({
                query: objs
            }, function (result) {
                if (result.Result) {
                    alert("更新成功！");
...
function startSearch() {
    ...
    beforeLoad();
    //todo:初始化查询参数
    window.baseTools.GetManagerWeibo(parameters, function (result) {//base.js //获取新闻舆情 
...
function initGridList(result) {
    ...
    ko.applyBindings(viewm, document.getElementById("weiboList"));
    if (result.rows.length == 0) {
        $("#weiboList tbody").empty().append("<tr><td colspan='7'>很抱歉，没有相关数据！</td></tr>");
    }

    $("#weiboList tfoot td").removeClass("loadingdata");
    $("#weiboList tbody").fadeIn(300);
}
//加载样式
function beforeLoad() {
    $("#weiboList tfoot td").addClass("loadingdata");
    $("#weiboList tbody").hide();
}
function endLoad() {
    $("#weiboList tfoot td").removeClass("loadingdata");
    $("#weiboList tbody").fadeIn(300);
